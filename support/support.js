...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const noticeContents = {
        ...
        "6": `<p>FOR-LOG 회원님들을 위한 혜택 업데이트 안내입니다.</p>
              <p>신규 가입 및 연속 로그인을 달성하신 회원님들을 위한 등급별 특별 포인트 지급 및 쿠폰 등 혜택이 강화되었습니다. 자세한 등급별 조건은 마이페이지 혜택 탭에서 확인하실 수 있습니다.</p>`,
        ...
        "4": `<p>개인정보보호법 개정에 따라 FOR-LOG의 개인정보처리방침이 일부 변경되었습니다.</p>
              <p>개정된 방침은 2026년 7월 1일부로 효력이 발생하며, 주요 개정 사항은 위탁 업체 변경 및 보안 시스템 강화에 따른 항목 추가입니다.</p>`,
              
        "3": `<p>회원님들께서 가장 많이 의견을 주셨던 상세 접근성 및 로딩 속도 관련 기능이 개선되었습니다.</p>
              <p>기존 대비 반응 속도가 약 30% 향상되었으며, 저사양 기기에서도 부드럽게 구동되도록 최적화 작업을 완료했습니다.</p>`,
              
        "2": `<p>모바일 환경 최적화 업데이트가 완료되었습니다.</p>
              <p>일부 OS에서 간헐적으로 발생하던 튕김 현상을 수정하고 UI 레이아웃을 좀 더 직관적으로 다듬었습니다. 스토어에서 최신 버전으로 업데이트 후 이용해 주세요.</p>`,
...
<div class="map-overlay-box">
    <div class="overlay-close-btn" onclick="window.closeCompanyWindow()">X</div>
...
// 키워드 검색 함수 실행 시 (회사 고정 핀과 말풍선을 모두 지도에서 숨김)
window.searchPlaces = function() {
    ...
    // 1) 검색창을 열면 메인 초록색 말풍선을 숨깁니다.
    if (typeof companyWindow !== 'undefined') {
        companyWindow.setMap(null);
    }
    
    // 2) 주변 맛집 클릭에 방해되지 않도록 회사 고정 핀도 지도에서 완전히 숨깁니다.
...
// 목록 우측 상단 X 버튼 클릭 시 작동하는 함수 (회사 위치 복원)
window.closePlacesList = function() {
    ...
    // X 버튼을 누르면 회사 고정 핀과 말풍선을 다시 지도에 나타나게 하고 중심으로 이동합니다.
    if (typeof companyMarker !== 'undefined') {
...
if (keywordInput) {
    ...
    window.searchPlaces = function() {
        alert('지도 서비스를 불러오지 못했습니다. 카카오 JavaScript 키의 등록 도메인을 확인해 주세요.');
        return false;
    };
    window.closePlacesList = function() {};
}
    ...
    const titleObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // 화면에 타이틀이 걸치면 show 클래스를 부여해서 애니메이션 작동
                entry.target.classList.add("show");
                // 한번 나타난 이후에는 다시 사라지지 않도록 관찰 종료
                observer.unobserve(entry.target);
            }
        });
    }, {
        root: null,
        threshold: 0.1 // 타이틀의 10%만 화면에 걸쳐도 즉시 부드럽게 나타남
    });
